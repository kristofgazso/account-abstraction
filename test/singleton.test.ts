  ...
  describe('Stake Management', () => {
    ...
    describe('without stake', () => {
      it('should return no stake', async () => {
        expect(await entryPoint.isStaked(addr, TWO_ETH)).to.eq(false)
      })
    ...
    describe('with stake of 2 eth', () => {
      ...
      it('should report "staked" state', async () => {
        expect(await entryPoint.isStaked(addr, TWO_ETH)).to.eq(true)
  ...
  describe('without paymaster (account pays in eth)', () => {
    describe('#handleOps', () => {
      ...
      it('should revert operation if wallet withdraw its stake', async function () {
        await wallet.addDeposit({value: ONE_ETH})
        const withdrawSender = Wallet.createRandom().address
        const withdraw = await entryPoint.populateTransaction.withdrawStake(withdrawSender).then(tx => tx.data!)
        const execWithdraw = await wallet.populateTransaction.exec(entryPoint.address, withdraw).then(tx => tx.data!)
        const execFromEntryPoint_withdraw = await wallet.populateTransaction.execFromEntryPoint(execWithdraw).then(tx => tx.data!)

        const op = await fillAndSign({
          sender: wallet.address,
          callData: execFromEntryPoint_withdraw,
          verificationGas: 1e6,
          callGas: 1e6
        }, walletOwner, entryPoint)
        const redeemerAddress = Wallet.createRandom().address
